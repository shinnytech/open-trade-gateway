...
void traderctp::ProcessOnFrontConnected()
{
	...
	Log(LOG_INFO,nullptr
		,"msg=ctp ProcessOnFrontConnected;key=%s;bid=%s;user_name=%s"
		,_key.c_str()
...
void traderctp::OnFrontConnected()
{
	...
	if (!m_b_login.load())
	{
		...
		ReqAuthenticate();
		Log(LOG_INFO,nullptr
			, "msg=ctp OnFrontConnected;key=%s;bid=%s;user_name=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str());
...
void traderctp::ProcessOnFrontDisconnected(int nReason)
{
	Log(LOG_WARNING,nullptr
		, "msg=ctp ProcessOnFrontDisconnected;key=%s;nReason=%d;bid=%s;user_name=%s"
		, _key.c_str()
		, nReason
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str());
...
void traderctp::OnFrontDisconnected(int nReason)
{
	...
	{
		Log(LOG_WARNING,nullptr
			, "msg=ctp OnFrontDisconnected;key=%s;nReason=%d;bid=%s;user_name=%s"
			, _key.c_str()
			, nReason
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str());
...
void traderctp::ProcessOnRspAuthenticate(std::shared_ptr<CThostFtdcRspInfoField> pRspInfo)
{
	...
	{
		Log(LOG_WARNING, nullptr
			,"msg=ctp OnRspAuthenticate;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrMsg=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
		...
		{
			Log(LOG_INFO, nullptr
				, "msg=ctp ProcessOnRspAuthenticate need ReinitCtp;key=%s;bid=%s;user_name=%s"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str());
	...
	, CThostFtdcRspInfoField* pRspInfo, int nRequestID, bool bIsLast)
{
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=ctp OnRspUserLogin;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrMsg=%s;TradingDay=%s;FrontId=%d;SessionId=%d;MaxOrderRef=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, pRspInfo ? pRspInfo->ErrorID : -999
		...
		{
			Log(LOG_WARNING,nullptr
				, "msg=ctp OnRspUserLogin;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrMsg=%s"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, pRspInfo ? pRspInfo->ErrorID : -999
...
{
	Log(LOG_INFO,nullptr 
		,"msg=ProcessOnRspUserLogin;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrMsg=%s;TradingDay=%s;FrontId=%d;SessionId=%d;MaxOrderRef=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
	...
	{
		Log(LOG_WARNING,nullptr
			, "msg=ctp ProcessOnRspUserLogin;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrMsg=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, pRspInfo ? pRspInfo->ErrorID : -999
		...
		{
			Log(LOG_INFO, nullptr
				, "msg=ctp ProcessOnRspUserLogin need ReinitCtp;key=%s;bid=%s;user_name=%s"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str());
			_ios.post(boost::bind(&traderctp::ReinitCtp, this));
	...
	else
	{
		...
		if (m_trading_day != trading_day)
		{
			...
			Log(LOG_INFO,nullptr
				, "msg=ctp reinit in new trading day;key=%s;bid=%s;user_name=%s;TradingDay_old=%s;TradingDay_new=%s"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, m_trading_day.c_str()
		...
		else
		{
			//正常的断开重连成功
			Log(LOG_INFO, nullptr
				,"msg=ctp reconnect success;key=%s;bid=%s;user_name=%s;TradingDay=%s"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, m_trading_day.c_str());
...
{
	Log(LOG_INFO,nullptr
		,"msg=ctp ProcessQrySettlementInfoConfirm;key=%s;bid=%s;user_name=%s;ConfirmDate=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
...
{
	Log(LOG_INFO,nullptr
		, "msg=ctp OnRspQrySettlementInfo,SettlementInfo is empty;key=%s;bid=%s;user_name=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str());
	m_need_query_settlement.store(false);
	...
	, bool bIsLast)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp ProcessSettlementInfoConfirm;key=%s;bid=%s;user_name=%s;bIsLast=%s;ConfirmDate=%s;ConfirmTime=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, bIsLast ? "true" : "false"		
...
{
	Log(LOG_INFO,nullptr
		, "msg=ctp OnRspUserPasswordUpdate;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrMsg=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
	...
	, std::shared_ptr<CThostFtdcRspInfoField> pRspInfo)
{
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=ctp OnRspOrderInsert;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrMsg=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, pRspInfo ? pRspInfo->ErrorID : -999
	...
	{
		Log(LOG_INFO, nullptr
			, "msg=ctp OnRspOrderInsert;key=%s;bid=%s;user_name=%s;ExchangeID=%s;InstrumentID=%s;OrderRef=%s;FrontId=%d;SessionId=%d;OrderPriceType=%c;Direction=%c;CombOffsetFlag=%c;LimitPrice=%f;VolumeTotalOriginal=%d;VolumeCondition=%c;TimeCondition=%c"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
...
{
	Log(LOG_INFO,nullptr
		, "msg=ctp OnRspOrderAction;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrMsg=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
	...
	std::shared_ptr<CThostFtdcRspInfoField> pRspInfo)
{
	...
	{
		Log(LOG_INFO,nullptr,
			"msg=ctp ProcessErrRtnOrderInsert;instance=%p;user_name=%s;ErrorID=%d;ErrMsg=%s"
			, _key.c_str()
			, _req_login.user_name.c_str()
			, pRspInfo ? pRspInfo->ErrorID : -999
			, pRspInfo ? GBKToUTF8(pRspInfo->ErrorMsg).c_str() : "");
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=ctp ProcessErrRtnOrderInsert;key=%s;bid=%s;user_name=%s;ExchangeID=%s;InstrumentID=%s;OrderRef=%s;FrontId=%d;SessionId=%d;OrderPriceType=%c;Direction=%c;CombOffsetFlag=%c;LimitPrice=%f;VolumeTotalOriginal=%d;VolumeCondition=%c;TimeCondition=%c"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, pInputOrder->ExchangeID
...
{
	Log(LOG_INFO,nullptr
		, "msg=ctp OnErrRtnOrderAction;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrorMsg=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
	...
	std::shared_ptr<CThostFtdcRspInfoField> pRspInfo, int nRequestID, bool bIsLast)
{
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=traderctp ProcessQryInvestorPosition;key=%s;bid=%s;user_name=%s;nRequestID=%d;bIsLast=%d;InstrumentId=%s;ExchangeId=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, nRequestID
	...
	std::shared_ptr<CThostFtdcRspInfoField> pRspInfo, int nRequestID, bool bIsLast)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=traderctp ProcessQryBrokerTradingParams;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrorMsg=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
	...
	Log(LOG_INFO,nullptr
		, "msg=traderctp BrokerTradingParams;key=%s;bid=%s;user_name=%s;Algorithm=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
	...
	std::shared_ptr<CThostFtdcRspInfoField> pRspInfo, int nRequestID, bool bIsLast)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=traderctp ProcessQryTradingAccount;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrorMsg=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
	...
	Log(LOG_INFO,nullptr
		, "msg=traderctp ProcessQryTradingAccount;key=%s;bid=%s;user_name=%s;available ctp return=%f"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, account.available);
...
{
	Log(LOG_INFO, nullptr
		, "msg=traderctp ProcessQryContractBank;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrorMsg=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
	...
	Log(LOG_INFO, nullptr
		, "msg=traderctp ProcessQryContractBank;key=%s;bid=%s;user_name=%s;bank_id=%s;bank_name=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, bank.bank_id.c_str()
...
{
	Log(LOG_INFO, nullptr
		, "msg=traderctp ProcessQryAccountregister;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrorMsg=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
...
{
	Log(LOG_INFO, nullptr
		, "msg=traderctp ProcessQryTransferSerial;key=%s;bid=%s;user_name=%s;ErrorID=%d;ErrorMsg=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pRspInfo ? pRspInfo->ErrorID : -999
	...
	std::shared_ptr<CThostFtdcRspTransferField> pRspTransfer)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp ProcessFromBankToFutureByFuture;key=%s;bid=%s;user_name=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str());
...
void traderctp::ProcessRtnOrder(std::shared_ptr<CThostFtdcOrderField> pOrder)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp OnRtnOrder;key=%s;bid=%s;user_name=%s;ExchangeID=%s;InstrumentId=%s;OrderRef=%s;FrontId=%d;SessionId=%d;OrderPriceType=%c;Direction=%c;CombOffsetFlag=%c;LimitPrice=%f;VolumeTotalOriginal=%d;TimeCondition=%c;VolumeCondition=%c;OrderLocalID=%s;OrderSubmitStatus=%c;OrderSysID=%s;OrderStatus=%c;VolumeTraded=%d;VolumeTotal=%d;InsertTime=%s;ZCETotalTradedVolume=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pOrder->ExchangeID
	...
	if (!ins)
	{
		Log(LOG_ERROR,nullptr
			, "msg=ctp OnRtnOrder,instrument not exist;key=%s;bid=%s;user_name=%s;symbol=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, order.symbol().c_str());
...
{
	Log(LOG_INFO,nullptr
		, "msg=ctp OnRtnTrade;key=%s;bid=%s;user_name=%s;ExchangeID=%s;InstrumentId=%s;OrderRef=%s;Direction=%c;OrderSysID=%s;OffsetFlag=%c;HedgeFlag=%c;Price=%f;Volume=%d;TradeDate=%s;TradeTime=%s;OrderSysID=%s;OrderLocalID=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pTrade->ExchangeID
	...
	{
		Log(LOG_ERROR,nullptr
			,"msg=ctp OnRtnTrade,instrument not exist;key=%s;bid=%s;user_name=%s;symbol=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, trade.symbol().c_str());
...
void traderctp::ProcessOnRtnTradingNotice(std::shared_ptr<CThostFtdcTradingNoticeInfoField> pTradingNoticeInfo)
{
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=ctp OnRtnTradingNotice;key=%s;bid=%s;user_name=%s;TradingNoticeInfoLength=%d"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, s.length());
...
void traderctp::ProcessRspError(std::shared_ptr<CThostFtdcRspInfoField> pRspInfo)
{
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=ctp OnRspError;key=%s;bid=%s;user_name=%s;ErrMsg=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
...
void traderctp::ReqAuthenticate()
{
	...
	{
		Log(LOG_INFO, nullptr
			, "msg=ctp ReqAuthenticate fail;key=%s;bid=%s;user_name=%s;UserProductInfo=%s;AuthCode=%s;ret=%d"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
...
void traderctp::SendLoginRequest()
{
	...
	{
		Log(LOG_INFO,nullptr
			,"msg=ctp ReqUserLogin fail;key=%s;bid=%s;user_name=%s;UserProductInfo=%s;LoginRemark=%s;ret=%d"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, _req_login.broker.product_info.c_str()
...
{
	Log(LOG_INFO,nullptr
		, "msg=ctp ReinitCtp begin;key=%s;bid=%s;user_name=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str());
	if (nullptr != m_pTdApi)
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp ReinitCtp end;key=%s;bid=%s;user_name=%s"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str());
}
...
void traderctp::ReqConfirmSettlement()
{
	...
	Log(LOG_INFO,nullptr
		,"msg=ctp ReqConfirmSettlement;key=%s;bid=%s;user_name=%s;ret=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, r);
...
void traderctp::ReqQrySettlementInfoConfirm()
{
	...
	Log(LOG_INFO,nullptr
		,"msg=ctp ReqQrySettlementInfoConfirm;key=%s;bid=%s;user_name=%s;ret=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, r);
...
int traderctp::ReqQryBrokerTradingParams()
{
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=ctp ReqQryBrokerTradingParams;key=%s;bid=%s;user_name=%s;ret=%d"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, r);
...
int traderctp::ReqQryAccount(int reqid)
{
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=ctp ReqQryTradingAccount;key=%s;bid=%s;user_name=%s;ret=%d"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, r);
...
int traderctp::ReqQryPosition(int reqid)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp ReqQryInvestorPosition;key=%s;bid=%s;user_name=%s;ret=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, r);
...
void traderctp::ReqQryBank()
{
	...
	Log(LOG_INFO,nullptr
		,"msg=ctp ReqQryContractBank;key=%s;bid=%s;user_name=%s;ret=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, r);
...
void traderctp::ReqQryAccountRegister()
{
	...
	Log(LOG_INFO, nullptr
		, "msg=ctp ReqQryAccountregister;key=%s;bid=%s;user_name=%s;ret=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, r);
...
void traderctp::ReqQrySettlementInfo()
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp ReqQrySettlementInfo;key=%s;bid=%s;user_name=%s;ret=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, r);
...
void traderctp::SendUserData()
{
		...
		it != m_data.m_positions.end(); ++it)
	{
		...
		{
			Log(LOG_ERROR,nullptr
				,"msg=ctp miss symbol %s when processing position;key=%s;bid=%s;user_name=%s"
				, symbol.c_str()
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
...
void traderctp::Start()
{
	...
	{
		Log(LOG_ERROR,nullptr
			, "msg=open message_queue;key=%s;errmsg=%s"
			,_key.c_str()
			,ex.what());
	}
	...
	{
		Log(LOG_ERROR, nullptr
			, "msg=trade ctp start ReceiveMsg thread fail;key=%s;errmsg=%s"
			, _key.c_str()
			, ex.what());
	}
}
...
void traderctp::ReceiveMsg(const std::string& key)
{
	...
	while (true)
	{
		...
		{
			Log(LOG_ERROR,nullptr
				, "msg=traderctp ReceiveMsg exception;key=%s;errmsg=%s"
				, strKey.c_str()
				, ex.what());
			break;
		}
...
{
	Log(LOG_INFO,nullptr
		, "msg=traderctp CloseConnection;key=%s;bid=%s;user_name=%s;conn id=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, nId);
...
void traderctp::ProcessInMsg(int connId, std::shared_ptr<std::string> msg_ptr)
{
	...
	{
		Log(LOG_WARNING,nullptr
			, "msg=traderctp parse json fail;key=%s;bid=%s;user_name=%s;msgcontent=%s;connid=%d"			
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, msg.c_str()
	...
	else if (req.aid == "change_password")
	{
		...
		{
			Log(LOG_ERROR,nullptr
				, "msg=trade ctp receive change_password msg before receive login msg;key=%s;bid=%s;user_name=%s;connid=%d"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, connId);
		...
		{
			Log(LOG_ERROR, nullptr
				, "msg=trade ctp receive change_password msg from a diffrent connection before login suceess;key=%s;bid=%s;user_name=%s;connid=%d"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, connId);
	...
	else
	{
		if (!m_b_login)
		{
			Log(LOG_WARNING,msg.c_str()
				,"msg=trade ctp receive other msg before login;key=%s;bid=%s;user_name=%s;connid=%d"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, connId);
		...
		{
			Log(LOG_WARNING,msg.c_str()
				, "msg=trade ctp receive other msg which from not login connecion;key=%s;bid=%s;user_name=%s;connid=%d"				
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, connId);
		...
		else if (aid == "confirm_settlement")
		{
			Log(LOG_INFO,msg.c_str()
				, "msg=trade ctp receive confirm_settlement;key=%s;bid=%s;user_name=%s;connid=%d"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, connId);
...
{
	Log(LOG_INFO,nullptr
		, "msg=traderctp ProcessReqLogIn;key=%s;bid=%s;user_name=%s;client_ip=%s;client_port=%d;client_app_id=%s;client_system_info=%s;front=%s;broker_id=%s"
		, _key.c_str()
		, req.bid.c_str()
		, req.user_name.c_str()
		, req.client_ip.c_str()
	...
	else
	{
		...
		{
			Log(LOG_INFO,nullptr
				, "msg=ctp login from custom front and broker_id;key=%s;bid=%s;user_name=%s;broker_id=%s;front=%s"
				, _key.c_str()
				, req.bid.c_str()
				, req.user_name.c_str()				
				, req.broker_id.c_str()
...
int traderctp::WaitLogIn()
{
	...
	if (0 == _logIn_status)
	{
		...
		{
			Log(LOG_WARNING,nullptr
				, "msg=CTP login timeout,trading fronts is closed or trading fronts config is error;key=%s;bid=%s;user_name=%s"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str());
		}
...
void traderctp::StopTdApi()
{
	...
	{
		Log(LOG_INFO, nullptr
			, "msg=ctp OnFinish;key=%s;bid=%s;user_name=%s"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str());
...
void traderctp::SendMsg(int connId, std::shared_ptr<std::string> msg_ptr)
{
	...
	if (totalLength > MAX_MSG_LENTH)
	{
		...
		{
			Log(LOG_ERROR,nullptr
				, "msg=SendMsg exception,%s;length=%d;key=%s;bid=%s;user_name=%s"
				, ex.what()
				, msg.length()
				, _key.c_str()
				, _req_login.bid.c_str()
...
void traderctp::OnClientReqChangePassword(CThostFtdcUserPasswordUpdateField f)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp ReqUserPasswordUpdate;key=%s;bid=%s;user_name=%s;ret=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, r);
...
void traderctp::OnClientReqTransfer(CThostFtdcReqTransferField f)
{
	...
	if (f.TradeAmount >= 0)
	{
		...
		Log(LOG_INFO,nullptr
			, "msg=ctp ReqFromBankToFutureByFuture;key=%s;bid=%s;user_name=%s;TradeAmount=%f;ret=%d"
			, _key.c_str()
			, _req_login.bid.c_str()
			, _req_login.user_name.c_str()
			, f.TradeAmount
	...
	else
	{
		...
		Log(LOG_INFO,nullptr
			, "msg=ctp ReqFromFutureToBankByFuture;key=%s;bid=%s;user_name=%s;TradeAmount=%f;ret=%d"
			, _key.c_str()
			, _req_login.bid.c_str()
			, f.UserID
			, f.TradeAmount
...
void traderctp::OnClientReqCancelOrder(CtpActionCancelOrder d)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp ReqOrderAction;key=%s;bid=%s;user_name=%s;InstrumentID=%s;OrderRef=%s;ret=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, d.f.InstrumentID
...
void traderctp::OnClientReqInsertOrder(CtpActionInsertOrder d)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctp ReqOrderInsert;key=%s;orderid=%s;bid=%s;user_name=%s;InstrumentID=%s;OrderRef=%s;ret=%d;OrderPriceType=%c;Direction=%c;CombOffsetFlag=%c;LimitPrice=%f;VolumeTotalOriginal=%d;VolumeCondition=%c;TimeCondition=%c"
		, _key.c_str()
