...
void traderctp::ProcessOnFrontConnected()
{
	...
	if (0 != ret)
	{
		Log(LOG_WARNING,nullptr
			,"msg=ctp ProcessOnFrontConnected;key=%s;bid=%s;user_name=%s;ret=%d"
			,_key.c_str()
			,_req_login.bid.c_str()
			,_req_login.user_name.c_str()
			,ret);
...
void traderctp::OnFrontConnected()
{
	...
	if (!m_b_login.load())
	{
		...
		{
			Log(LOG_WARNING, nullptr
				, "msg=ctpse OnFrontConnected;key=%s;bid=%s;user_name=%s;ret=%d"
				, _key.c_str()
				, _req_login.bid.c_str()
				, _req_login.user_name.c_str()
				, ret);
	...
	std::shared_ptr<CThostFtdcRspInfoField> pRspInfo)
{
	...
	{
		Log(LOG_INFO,nullptr
			, "msg=ctpse ProcessErrRtnOrderInsert;key=%s;user_name=%s;ErrorID=%d"
			, _key.c_str()
			, _req_login.user_name.c_str()
			, pRspInfo ? pRspInfo->ErrorID : -999);
	}
...
void traderctp::ProcessRtnOrder(std::shared_ptr<CThostFtdcOrderField> pOrder)
{
	...
	Log(LOG_INFO,nullptr
		, "msg=ctpse OnRtnOrder;key=%s;bid=%s;user_name=%s;InstrumentId=%s;OrderRef=%s;Session=%d;OrderPriceType=%c;Direction=%c;CombOffsetFlag=%c;LimitPrice=%f;VolumeTotalOriginal=%d;TimeCondition=%c;VolumeCondition=%c;OrderLocalID=%s;OrderSubmitStatus=%c;OrderSysID=%s;OrderStatus=%c;VolumeTraded=%d;VolumeTotal=%d;InsertTime=%s;ZCETotalTradedVolume=%d"
		, _key.c_str()
		, _req_login.bid.c_str()
		, _req_login.user_name.c_str()
		, pOrder->InstrumentID
